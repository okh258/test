...
func TestWsServer(t *testing.T) {
	...
	http.HandleFunc("/ws", func(w http.ResponseWriter, req *http.Request) {
		...
		if l := len(uid); l < 10 || l > 64 {
			log.Println("uid illegal, uid: ", uid)
			wsjson.Write(req.Context(), conn, "非法uid，uid长度：10-64")
			conn.Close(websocket.StatusUnsupportedData, "uid illegal!")
			return
		}
